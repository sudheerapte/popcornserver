...
class WebsocketEmitter extends EventEmitter {
  ...
  _readData(data) {
    ...
    log(`offset = ${offset}, data.length = ${data.length}`);
  ...
  _processOp(opcode, payload) {
    ...
    if (opcode === 0 || opcode === 1) {
      log(`_processOp emitting message: |${payload.toString().substr(0,10)}...|`);
  ...
  sendMessage(buf, cb) {
    ...
    log(`sendMessage: writing frame length ${frame.length}`);
    const done = this._writeStream.write(frame);
    if (done) {
      cb();
    } else {
  ...
  _fillFrame(opcode, buf, masked) {
    ...
    log(`_fillFrame encoding payload for buf length ${len}`);
    let extpayloadlen = 0;
    let offset = 2;
...
require('./debug-log.js')
  .registerLogger('websocket-emitter', logger);
